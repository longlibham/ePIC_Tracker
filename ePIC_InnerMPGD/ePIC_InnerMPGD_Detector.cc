...
void ePIC_InnerMPGD_Detector::ConstructMe(G4LogicalVolume* logicWorld){
    ...
    if(!std::isfinite(radius) || !std::isfinite(coat_t) || !std::isfinite(z_min) || !std::isfinite(z_max) || 
        ...
        !std::isfinite(DriftKapton_t) || !std::isfinite(DriftCuGround_t) || !std::isfinite(fudge_t)){
            ...
            cout<<"radius: "<< radius << " mpgd_t: "<<coat_t<<" z_min: "<<z_min<<" z_max: "<<endl;
...
//     // construct the gas cylinder to contain the inner MPGD detector
    ...
    //drift copper ground
    double width_nooverlap = 0.01*cm;
    // double radius_nooverlap = 0.02*cm;
    G4Material* mat_cu = nist->FindOrBuildMaterial("G4_Cu");
    G4Colour col_cu = G4Colour(221./255., 125./255., 4./255., 1.);
    double width = radius*std::tan(2*M_PI/stave_number) - width_nooverlap;
    ...
    G4VisAttributes* DriftKapton_vis = new G4VisAttributes(col_kapton);
    ...
    G4VisAttributes* DriftCuElectrode_vis = new G4VisAttributes(col_cu);
    ...
    G4VisAttributes* gasgap_vis = new G4VisAttributes(col_mpgd_gas);
    ...
    G4VSolid* mesh_solid = new G4Box("mesh_plane", width/2., mesh_t/2., (z_max-z_min)/2.);
    G4LogicalVolume* mesh_logic = new G4LogicalVolume(mesh_solid, mat_mesh, "MeshLogic");
    G4VisAttributes* mesh_vis = new G4VisAttributes(col_inox);
    ...
    G4VisAttributes* fudge_vis = new G4VisAttributes(col_kapton);
    ...
    G4VisAttributes* ResistiveStrip_vis = new G4VisAttributes(col_carbon);
    ...
    G4VisAttributes* CuStrip_vis = new G4VisAttributes(col_cu);
    ...
    G4VisAttributes* CuGround_vis = new G4VisAttributes(col_cu);
    ...
    G4VisAttributes* KaptonOverlay_vis = new G4VisAttributes(col_kapton);
    ...
    for(int istave =0; istave<stave_number; istave++){
        ...
        //Drift Kapton
        double radius_dk = radius + DriftCuGround_t/2. + DriftKapton_t/2.;
        ...
        //Drift Cu Electrode
        double radius_dce = radius_dk + DriftKapton_t/2. + DriftCuElectrode_t/2.;
        ...
        //gas gap
        double radius_gg = radius_dce + DriftCuElectrode_t/2. + GasGap_t/2.;
        ...
        //Mesh
        double radius_mesh = radius_gg + GasGap_t/2. + mesh_t/2.;
        ...
        //fudge
        double radius_fudge = radius_mesh + mesh_t/2. + fudge_t/2.;
        ...
        //gas
        double radius_g = radius_fudge + fudge_t/2. + gas_t/2.;
        ...
        //Resistive strip
        double radius_rs = radius_g + gas_t/2. + ResistiveStrip_t/2.;
        ...
        //Kapton strip
        double radius_ks = radius_rs + ResistiveStrip_t/2. + KaptonStrip_t/2.;
        ...
        //Cu strip
        double radius_cus = radius_ks + KaptonStrip_t/2. + CuStrip_t/2.;
        ...
        //PCB
        double radius_pcb = radius_cus + CuStrip_t/2. + pcb_t/2.;
        ...
        //Cu ground
        double radius_cug = radius_pcb + pcb_t/2. + CuGround_t/2.;
        ...
        //Kapton Overlay
        double radius_ko = radius_cug + CuGround_t/2. + KaptonOverlay_t/2.;
...
void ePIC_InnerMPGD_Detector::Print(const string& what)const{
