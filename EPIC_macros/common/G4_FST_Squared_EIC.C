...
#include "/home/scm/Fun4All/myinstall/include/squareddisk/SquaredDiskSubsystem.h"
...
R__LOAD_LIBRARY(libSquaredDisk.so)
...
void FSTSetup(PHG4Reco *g4Reco)
{
  ...
  gSystem->Load("libSquaredDisk.so");
...
// Positive Disks
  const double pos_z[] = {25, 45, 70, 100, 135, -25, -45, -65, -90, -115};
  double rmin[] = {3.68, 3.68, 3.79, 4.56, 5.41, 3.68, 3.68, 3.68, 3.92, 4.30};
  double rmax[] = {23, 43, 43, 43, 43, 23, 43, 43., 43, 43.};
  double x_offset[] = {0, 0, 0.055, 0.78, 1.6, 0, 0., 0., 0.082, 0.33};





  SquaredDiskSubsystem *squareddisk;

  for (int i{0}; i<10; i++){
    std::string diskName = "FBVS_" + std::to_string(i);
    squareddisk = new SquaredDiskSubsystem(diskName);
    squareddisk->set_string_param("material", "G4_Si");
    squareddisk->set_double_param("x_offset", x_offset[i]);
    squareddisk->set_double_param("r_min", rmin[i]);
    squareddisk->set_double_param("r_max", rmax[i]);
    squareddisk->set_double_param("place_z", pos_z[i]);
    squareddisk->set_double_param("rot_z", 180.);
    squareddisk->SetActive();
    squareddisk->SuperDetector(diskName);
    
    g4Reco->registerSubsystem(squareddisk);
  }
