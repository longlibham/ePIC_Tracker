...
void FTTLSetup(PHG4Reco *g4Reco, TString fttloption = "")
{
  ...
  for (Int_t i = 0; i < G4TTL::layer[2]; i++){
    ...
    if(!G4TTL::SETTING::optionBasicGeo){
      make_forward_station(Form("FTTL_%d", i), g4Reco, G4TTL::positionToVtx[2][i],  G4TTL::minExtension[2][i], G4TTL::maxExtension[2][i], 300*um, Enable::IP8 ? G4TTL::xoffsetFTTLIP8[i] : G4TTL::xoffsetFTTLIP6[i]);
    } else {
      make_forward_station_basic(Form("FTTL_%d", i), g4Reco, G4TTL::positionToVtx[2][i],  G4TTL::minExtension[2][i], G4TTL::maxExtension[2][i], 300*um);
    }
  }
}
...
void ETTLSetup(PHG4Reco *g4Reco, TString ettloption = "")
{
  ...
  for (Int_t i = 0; i < G4TTL::layer[0]; i++){
    ...
    if(!G4TTL::SETTING::optionBasicGeo){
      make_forward_station(Form("ETTL_%d", i), g4Reco, G4TTL::positionToVtx[0][i],  G4TTL::minExtension[0][i], G4TTL::maxExtension[0][i], 300*um, 0);
    } else {
      make_forward_station_basic(Form("ETTL_%d", i), g4Reco, G4TTL::positionToVtx[0][i],  G4TTL::minExtension[0][i], G4TTL::maxExtension[0][i], 300*um);
    }
  }
}
  ...
  for (Int_t i = 0; i < G4TTL::layer[1]; i++){
    cout << "Radius: " << G4TTL::positionToVtx[1][i] << "\tLength: " << G4TTL::minExtension[1][i] << "\tz-Offset: " << G4TTL::maxExtension[1][i] << endl;
     if(G4TTL::SETTING::optionBasicGeo && G4TTL::SETTING::optionLYSO){
      make_barrel_layer_LYSO_basic(Form("CTTL_%d",i), g4Reco, G4TTL::positionToVtx[1][i],  G4TTL::minExtension[1][i], 300*um, G4TTL::maxExtension[1][i]);
    } else if(G4TTL::SETTING::optionBasicGeo){
      make_barrel_layer_basic(Form("CTTL_%d",i), g4Reco, G4TTL::positionToVtx[1][i],  G4TTL::minExtension[1][i], 300*um, G4TTL::maxExtension[1][i]);
    } else {
      make_barrel_layer(Form("CTTL_%d",i), g4Reco, G4TTL::positionToVtx[1][i],  G4TTL::minExtension[1][i], 300*um, G4TTL::maxExtension[1][i]);
    }
  }
}
                      ...
                      double radius, double halflength, double tSilicon, double zOffset )
{
  ...
  if (TRACKING::FastKalmanFilter)
  {
    TRACKING::FastKalmanFilter->add_phg4hits(string("G4HIT_") + name,     //      const std::string& phg4hitsNames,
                                             ...
                                             G4TTL::PositionResolution,     //      const float phires,
                                             100*G4TTL::PositionResolution,     //      const float lonres,
                                             0.5,                           //      const float eff,
                                             0);                          //      const float noise
    cout << "USING 30x3000um ToF reso" << endl;
  ...
  if (TRACKING::FastKalmanFilterInnerTrack)
  {
    TRACKING::FastKalmanFilterInnerTrack->add_phg4hits(string("G4HIT_") + name,     //      const std::string& phg4hitsNames,
                                             ...
                                             0.5,                           //      const float eff,
                                             0);                          //      const float noise
    cout << "USING 30x3000um ToF reso (inner track)" << endl;
...
//     cout << currRadius << endl;
  ...
  if (TRACKING::FastKalmanFilter)
  {
    ...
    TRACKING::FastKalmanFilter->add_phg4hits(string("G4HIT_") + name,     //      const std::string& phg4hitsNames,
                                             ...
                                             0);                          //      const float noise
    cout << "USING RES LGAD BARREL" << endl;
    TRACKING::FastKalmanFilter->add_cylinder_state(name, radius);
