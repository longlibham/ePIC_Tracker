...
namespace RWELL
{
  ...
  const int n_layer = 2;  //tracker layers
  const double nom_radius[RWELL::n_layer] = {55., 73.};
  // double e_length_uRwell[RWELL::n_layer] =  {106., 197};
  double e_length_uRwell[RWELL::n_layer] =  {115., 197};
                           ...
                           int index = 0)
{
  ...
  double fudge_factor_X0 = 0.25;// Fudge factor of 0.25% X/X0 of Cu to make material correct
  double fudge_factor_thickness = fudge_factor_X0*0.01*1.436;// X0 Cu = 1.436cm
  //Cu
                            ...
                            int index = 0)
{
  ...
  rwell_cyl->set_double_param("radius", rsum);
  rwell_cyl->set_string_param("material", "G4_Cu");
  rwell_cyl->set_double_param("thickness", cu_thickness+fudge_factor_thickness);
  rwell_cyl->set_int_param("lengthviarapidity", 0);
  rwell_cyl->set_double_param("place_z", Shift);
  rwell_cyl->set_double_param("length", length);
  rwell_cyl->SuperDetector("RWELL");
                  ...
                  int type = 1)
{
  ...
  //for (int ilyr = 0; ilyr < 1; ilyr++)  //RWELL trackers are registered in Build_RWELL macro
    {
      if (type == 0)
	{
      ...
      if (type == 1)
	{
      ...
      if (TRACKING::FastKalmanFilter and RWELL::nom_radius[ilyr] < 70)
	{
