...
#ifdef _SIVTX_
  for (int i{0}; i<3; i++){
    std::string vtxName = "G4HIT_SVTX_" + std::to_string(i);
    //std::string vtxName = "G4HIT_SVTX_";
  // add Vertexing Layers
  kalman->add_phg4hits(
		       vtxName,				// const std::string& phg4hitsNames,
...
#ifdef _SIBARR_
for (int i{0}; i<2; i++){
  std::string barrName = "G4HIT_BARR_" + std::to_string(i);
  // add Barrel Layers
  kalman->add_phg4hits(
		       barrName,                   	// const std::string& phg4hitsNames,
		       PHG4TrackFastSim::Cylinder,
		       999.,                           	// radial-resolution [cm]
...
#ifdef _SIDISKS_
for (int i{0}; i<10; i++){
  std::string diskName = "G4HIT_FBST_" + std::to_string(i);
  //  add Disk Layers
  kalman->add_phg4hits(
