...
def read_tree(Tfile, outfile):
    ...
    bins = 200
    gp = float(outfile.split('-')[-1].split('G')[0])
    ...
    c1.Divide(2, 2)
    c1.cd(1)
    frp_p = h1.Fit('gaus','S', '', h1.GetMean()-3*h1.GetRMS(), h1.GetMean()+3*h1.GetRMS())
    ...
    frp_dcat = h3.Fit("gaus", 'S', '', h3.GetMean()-3*h3.GetRMS(), h3.GetMean()+3*h3.GetRMS())
    h3.Draw()
    c1.cd(4)
    frp_dcaz = h4.Fit("gaus", 'S', '', h4.GetMean()- 3*h4.GetRMS(), h4.GetMean()+ 3*h4.GetRMS())
    h4.Draw()
...
def process_tree(args):
    ...
    ge4.Draw("AP")
    c2.Print(f"plots/Summary_plot_{config}_{fixed}.pdf")
